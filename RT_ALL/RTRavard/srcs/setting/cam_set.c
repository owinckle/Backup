...
static void	cam_shift(t_env *e)
{
	...
	if (e->move[6])
		vect_sub(e->c->pos, e->c->pos, vect_multi(tmp, e->move[12] * PAS, e->c->base[0]));
	if (e->move[7])
		vect_sum(e->c->pos, e->c->pos, vect_multi(tmp, e->move[12] * PAS, e->c->base[0]));
	if (e->move[8])
		vect_sub(e->c->pos, e->c->pos, vect_multi(tmp, e->move[12] * PAS, e->c->base[2]));
	if (e->move[9])
		vect_sum(e->c->pos, e->c->pos, vect_multi(tmp, e->move[12] * PAS, e->c->base[2]));
	if (e->move[10])
		vect_sum(e->c->pos, e->c->pos, vect_multi(tmp, e->move[12] * PAS, e->c->base[1]));
	if (e->move[11])
		vect_sub(e->c->pos, e->c->pos, vect_multi(tmp, e->move[12] * PAS, e->c->base[1]));
}
