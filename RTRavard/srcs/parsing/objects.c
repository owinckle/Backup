...
static t_obj	*col_pos(t_env *e, t_obj *link, int j)
{
	...
	if (ft_strncmp(e->scene[j + 2], "		col(", 6) != 0 ||
		...
		ft_error(FILE);
	link->col.red = ft_atoin(e->scene[j + 2], &i);
	link->col.green = ft_atoin(e->scene[j + 2], &i);
	link->col.blue = ft_atoin(e->scene[j + 2], &i);
	if (e->scene[j + 2][i] != ')')
		ft_error(FILE);
	i = 6;
